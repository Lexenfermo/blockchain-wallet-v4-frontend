...
;(async () => {
  ...
  const [options, mainProcess, securityProcess] = await Promise.all([
    (await fetch('/Resources/wallet-options-v4.json')).json(),
    createProcess({ name: `main`, src: `/main.html/#/login` }),
    createProcess({ name: `security`, src: `/security.html/#${pathname}` })
  ])
